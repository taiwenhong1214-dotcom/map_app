...
class _PlannerPageState extends ConsumerState<PlannerPage>
  ...
  // 动画状态：Live Tracking 幽灵插值
  late AnimationController _peerAnimController;
  final Map<String, LatLng84> _currentPeerPositions = {};
  final Map<String, LatLng84> _targetPeerPositions = {};
  ...
  @override
  void dispose() {
    _peerAnimController.dispose();
    super.dispose();
  }

  LatLng84 _interpolatePeerPos(String userId) {
    final start = _currentPeerPositions[userId];
    final end = _targetPeerPositions[userId];
    if (start == null || end == null) {
      return end ?? start ?? const LatLng84(0, 0);
    }

    final t = _peerAnimController.value;
  ...
  Widget build(BuildContext context) {
    ...
    // 监听实时位置更新，触发插值动画
    ref.listen(liveTrackingProvider, (previous, next) {
      bool changed = false;
      for (var peer in next.peers) {
        if (_targetPeerPositions[peer.userId] != peer.position) {
          final startPos = _currentPeerPositions[peer.userId];
          if (startPos == null && _targetPeerPositions[peer.userId] == null) {
            // First time seeing this peer
            _currentPeerPositions[peer.userId] = peer.position;
          } else {
            _currentPeerPositions[peer.userId] = _interpolatePeerPos(peer.userId);
          }
          _targetPeerPositions[peer.userId] = peer.position;
          changed = true;
        }
      }
      if (changed) {
      ...
      if (liveState.isConnected && !liveState.isHost) {
        if (next.hasValue) {
          if (next.value == null) {
            ...
            // 同步房主的新行程
            try {
              final itinerary = Itinerary.fromJson(next.value!);
              ref.read(currentItineraryNotifierProvider.notifier).setItinerary(itinerary);
            } catch (e) {
              print('Error parsing synced itinerary: $e');
            }
          }
        }
      }
    ...
    // 监听本地行程，同步到云端 (Host)
    ref.listen(currentItineraryNotifierProvider, (previous, next) {
      final liveState = ref.read(liveTrackingProvider);
      if (next.value != null && liveState.isConnected && liveState.isHost) {
        ref.read(liveTrackingRepositoryProvider).syncItinerary(next.value!.toJson());
      }
    });

    Map<String, dynamic>? routeData;
    if (itineraryState.value != null) {
      final allPois = itineraryState.value!.days
          .expand((day) => day.pois)
          .toList();
      if (allPois.length >= 2) {
        final coordsStr = allPois
                ...
                child: InkWell(
                  ...
                  onTap: () {
                    ...
                    if (liveState.isConnected) {
                      ref.read(liveTrackingProvider.notifier).disconnect();
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text(strings.exitRoomMsg)),
                      );
                    }
                    // 清空数据，回到输入表单
                    ref.read(currentItineraryNotifierProvider.notifier).clear();
          ...
          Positioned(
            ...
            child: Column(
              ...
              children: [
                ...
                // 回到目的地行程按钮
                if (itineraryState.value != null &&
                    itineraryState.value!.days.isNotEmpty) ...[
                  FloatingActionButton.small(
                    heroTag: 'btn_memory',
                    backgroundColor: Theme.of(context).cardColor,
                    elevation: 2,
                    ...
                    onPressed: () async {
                      HapticFeedback.lightImpact();
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text(strings.scanningMemories)),
                      );
                      final repo = ref.read(memoryRepositoryProvider);
                      final results = await repo.matchPhotosToItinerary(
                        itineraryState.value!,
                      );
                      ref
                          .read(photoFootprintsProvider.notifier)
                          .setFootprints(results);
                      if (results.isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text(strings.noMemories)),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              strings.memoriesLit.replaceAll(
                                '{0}',
                  ...
                  FloatingActionButton.small(
                    heroTag: 'btn_live_tracking',
                    backgroundColor: Theme.of(context).cardColor,
                    elevation: 2,
                    child: Icon(
                      Icons.group_add,
                      color: ref.watch(liveTrackingProvider).isConnected
                          ? Colors.green
                          : Colors.orangeAccent,
                    ),
                    onPressed: () {
                      HapticFeedback.mediumImpact();
                      _showLiveTrackingSheet(context, itineraryState.value!.id);
                    },
                  ),
                  const SizedBox(height: 12),
                  FloatingActionButton.small(
                    ...
                    onPressed: () {
                      HapticFeedback.lightImpact();
                      final dest =
                          itineraryState.value!.days.first.pois.first.location;
          ...
          SafeArea(
            child: itineraryState.when(
              data: (itinerary) {
                if (itinerary == null) {
                  if (ref.watch(liveTrackingProvider).isConnected) {
                    return const SizedBox.shrink(); // 隐藏表单，仅显示地图
                  }
                  return Align(
                      ...
                      children: [
                        Padding(
                          ...
                          child: FloatingActionButton.extended(
                            heroTag: 'btn_join_room_topleft',
                            backgroundColor: Colors.green,
                            elevation: 2,
                            icon: const Icon(Icons.login, color: Colors.white, size: 18),
                            label: Text(strings.joinRoomBtn, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                            onPressed: () {
                              HapticFeedback.mediumImpact();
                              _showJoinRoomSheet(context);
                            },
                  ...
                  return Container(
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        Text(
                          ref.watch(generationStatusProvider) ??
                              strings.aiLoading,
  ...
  Widget _buildMapLayer(
    ...
    // 🌟 处理 Live Tracking 的好友插值动画
    final liveState = ref.watch(liveTrackingProvider);
    if (liveState.isConnected) {
      for (var peer in liveState.peers) {
        markers.add(
          TravelMapMarker(
            id: peer.userId,
            position: _interpolatePeerPos(peer.userId),
            label: peer.userName,
          ),
        );
      }
    ...
    if (routeData != null &&
        routeData['points'] != null &&
        (routeData['points'] as List<LatLng84>).isNotEmpty) {
    ...
    final int poiCount =
        itinerary?.days.fold<int>(
          0,
          (int prev, day) => prev + day.pois.length,
        ) ??
        0;
    ...
    return TravelMapFactory.build(
      key: ValueKey(
        '${itinerary?.id ?? 'default_map'}_${itinerary?.days.length ?? 0}_${poiCount}_$routeKey',
      ),
      initialCenter: itinerary != null && itinerary.days.isNotEmpty
          ? itinerary.days.first.pois.first.location
          : center,
      destinationForEngineDecision:
          itinerary != null && itinerary.days.isNotEmpty
          ? itinerary.days.first.pois.first.location
          : center,
      initialZoom: 13.0,
      markers: markers,
                              ...
                              children: [
                                if (itinerary.localId == null) ...[
                                  ElevatedButton.icon(
                                    ...
                                    onPressed: () async {
                                      ...
                                      await ref
                                          .read(
                                            currentItineraryNotifierProvider
                                                .notifier,
                                          )
                                          .saveToLocal();
                                      ScaffoldMessenger.of(
                                        ...
                                        SnackBar(
                                          content: Text(strings.savedToLocal),
                                        ),
                                      );
                                    },
                                  ),
                                ...
                                ElevatedButton.icon(
                                  ...
                                  onPressed: () {
                                    ...
                                    final footprints = ref.read(
                                      photoFootprintsProvider,
                                    );
                                    ItineraryPosterGenerator.sharePoster(
                                      ref,
                                      context,
                                      itinerary,
                                      ref.read(photoFootprintsProvider),
                                    );
                                  },
                                ),
                              ],
        ...
        child: Column(
          ...
          children: [
            ...
            ...day.pois.asMap().entries.map((entry) {
              ...
              String trafficText = routeData == null
                  ? strings.trafficLoading
                  : strings.trafficLocal;
              ...
              if (routeData != null && routeData['legs'] != null) {
                ...
                if (currentGlobalIndex < legs.length) {
                  ...
                  final distanceKm = (leg['distance'] / 1000).toStringAsFixed(
                    1,
                  );
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Builder(
                              builder: (context) {
                                final footprint = footprints
                                    .where((f) => f.poi.id == poi.id)
                                    .firstOrNull;
                                if (footprint != null) {
                                  return Row(
                                    children: [
                                  ...
                                  return GestureDetector(
                                    onTap: () async {
                                      ...
                                      if (asset != null) {
                                        ...
                                        final fp = await repo
                                            .createFootprintForPoi(poi, asset);
                                        if (fp != null) {
                                          ref
                                              .read(
                                                photoFootprintsProvider
  ...
  void _showLiveTrackingSheet(BuildContext context, String _ignoredRoomId) {
    final strings = AppStrings(ref.read(localeProvider));
    final liveState = ref.read(liveTrackingProvider);
    final isConnected = liveState.isConnected;
    final roomId = isConnected && liveState.roomId != null 
        ? liveState.roomId! 
        : (100000 + Random().nextInt(900000)).toString();
    final inviteCode = roomId;
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              const Icon(Icons.group_add, size: 48, color: Colors.green),
              const SizedBox(height: 16),
              Text(
                strings.liveTrackingTitle,
                style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 8),
              Text(
                strings.liveTrackingDesc,
                textAlign: TextAlign.center,
              ...
              TextButton(
                onPressed: () {
                  HapticFeedback.lightImpact();
                  Clipboard.setData(
                    ClipboardData(text: strings.inviteMessage(inviteCode)),
                  );
                  Navigator.pop(ctx);
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(strings.inviteCopiedMsg)),
                  );
                },
                child: Text(strings.copyInviteCode),
              ),
            ],
          ),
        ),
              ...
              child: SafeArea(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 24),
                    const Icon(Icons.door_front_door, size: 48, color: Colors.green),
                    const SizedBox(height: 16),
                    Text(strings.joinLiveTrackingRoom, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    Text(strings.enterInviteCodeDesc, textAlign: TextAlign.center, style: const TextStyle(color: Colors.grey)),
                    const SizedBox(height: 24),
                    TextField(
                      ...
                      style: TextStyle(fontSize: 32, fontWeight: FontWeight.w900, letterSpacing: 16, color: Theme.of(context).colorScheme.primary),
                      decoration: InputDecoration(
                        hintText: strings.enterCodeHint,
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: isLoading ? null : () async {
                          ...
                          // 尝试加入房间
                          final success = await ref.read(liveTrackingProvider.notifier).connect(
                            codeController.text, 
                            randomId, 
                            randomName, 
                            null
                          ...
                          if (context.mounted) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(strings.joinedDemoMsg)));
                          }
                        },
                        child: isLoading 
                          ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                          : Text(strings.enterRoomSubmitBtn, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ),
                    ),
